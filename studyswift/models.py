...
class Homework(models.Model):
    ...
    assigned_class = models.ForeignKey(
        SchoolClass, on_delete=models.CASCADE, related_name="homework"
    )
    teacher = models.ForeignKey(User, on_delete=models.CASCADE)
    due_date = models.DateTimeField(default=datetime.now() + timedelta(days=7))
    files = models.ManyToManyField(
        HomeworkFile, related_name="homework_files", blank=True
    )
    students = models.ManyToManyField(
        User, through="HomeworkSubmission", related_name="homeworks"
    )
...
class HomeworkSubmission(models.Model):
    ...
    files = models.ManyToManyField(
        HomeworkFile, related_name="submission_files", blank=True
    )
